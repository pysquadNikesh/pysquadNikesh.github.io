...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const phone = this.document.querySelector(".phone");
  ...
  phone.href = `tel:${UserData[4].phone}`;
  address.textContent = `${UserData[5].location}`;
  phone.textContent = `${UserData[4].phone}`;
  designation.innerHTML = `${UserData[6].designation}`;
  ...
  certificates.forEach((certificate, index) => {
    const certificateLi = this.document.createElement("li");
    certificateLi.classList.add("certificate-item", "active");
    certificateLi.setAttribute("data-filter-item", "");

    certificateLi.innerHTML = `
    <a href="#">
        <figure class="certificate-img">
          <div class=" certificate-item-icon-box" data-certi-index="${index}">
            <ion-icon name="eye-outline"></ion-icon>
          </div>
    ...
    fragment.appendChild(certificateLi);
  });
  document.querySelector(".certificate-list").appendChild(fragment);
  ...
  modalOpen.forEach((link) => {
    link.addEventListener("click", function (e) {
      ...
      if (project.technologies) {
        techTitle.innerHTML = "Technologies";
      } else {
        techTitle.innerHTML = "";
      }
  ...
  const certificateModal = document.querySelectorAll(
    ".certificate-item-icon-box"
  );

  certificateModal.forEach((link) => {
    link.addEventListener("click", function (e) {
      e.preventDefault();
      const index = this.getAttribute("data-certi-index");
      const certificate = certificates[index];
      console.log(certificates[index]);
      console.log("click");

      if (certificate.technologies) {
        techTitle.innerHTML = "Technologies";
      } else {
        techTitle.innerHTML = "";
      }
      ...
      modalImage.setAttribute("src", certificate.image);
      modalTitle.innerText = certificate.name;
      modalDetails.innerText = "";
      techDetails.textContent = "";
      // modalDetails.innerText = certificate.description;
